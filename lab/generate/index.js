...
{
		var proData = 
		[{ id: "USER_TABLE", text: '用户表' }, 
		{ id: "VIEW", text: '视图'}];
		$("#txtPro").ligerComboBox({ data: proData, isMultiSelect: false,
				onSelected: function (newvalue)
				{
					$.getJSON('getTables.asp?ProcessType=getTables',{type_desc:newvalue},function(data){
							$("#txtCity").ligerGetComboBoxManager().setData(data);
						});
				}
		});
		$("#txtCity").ligerComboBox({ data: null, isMultiSelect: false,
			onSelected: function (newvalue){
					$.getJSON('getTables.asp?ProcessType=getTableDetail',{TableName:newvalue},function(data){
/*						if($('#showTable')[0]){
            var jsonObj = {};
            jsonObj.Rows = data
...
function bulidMainGrid(columns)
{
    ...
    $(columns).each(function ()
    {
		...
		var gridPanle;
		if($('#showTable')[0])gridPanle=$('#showTable');
    else gridPanle = $('<div style="margin:7px;" id="showTable"></div>').appendTo('body');
...
function createGridToolbar(tName)
{
    ...
    items.push({ text: 'SERVER段代码', click: outserver, img: "../icons/printer_48.png" });
    ...
    function outserver()
    {
        ...
        {
            var name = window.grid.rows[i].name;
						serverstr+="rs(\""+name+"\")=Request.Form(\""+name+"\")\n";
        }
        var textarea = $("<textarea />").width(400).height(120);
        textarea.val(serverstr);
        $.ligerDialog.open({
            title: 'SERVER',
            target: textarea.wrap('<div></div>').parent().css('margin', 10),
            width: 470, height: 200, isResize: true,
