...
(function ($)
{
    ...
    $.ligerDefaults.ComboBox = {
        ...
        absolute: true         //选择框是否在附加到body,并绝对定位
				//add by zbh 2012-09-05
				,inputReadOnly:true //默认输入区域只读属性
    };
    ...
    $.ligerui.controls.ComboBox.ligerExtend($.ligerui.controls.Input, {
        ...
        _render: function ()
        {
            ...
            {
							//modify by zbh 2012-09-04
							//修改下拉框可以编辑，根据输入的内容从下拉框中选择，找不到就清空
                this.element.readOnly = p.inputReadOnly;
                g.inputText = $(this.element);
                g.textFieldID = this.element.id;
								//添加失去焦点触发
//								g.inputText.blur(function() {
//									g.options.initValue=g.inputText.val()||g.inputText;
//									g.updateStyle();
//									if(g.inputText&&g.textFieldID){
//										var z = $.ligerui.get(g.textFieldID);
//										z.inputText=g.inputText;
//									}
//								});
            }
        ...
        selectValueByGrid: function (value)
        {
            ...
            if (value != null)
            {
                ...
                $(valuelist).each(function (i, item)
                {
                    for(var z=0;z<g.gridManager.data.Rows.length;z++){
											if(g.gridManager.data.Rows[z][p.valueField]==item.toString())text+=g.gridManager.data.Rows[z][p.textField];
										};
                    if (i < valuelist.length - 1) text += p.split;
                });
                g._changeValue(value, text);
        ...
        _dataInit: function ()
        {
            ...
            else if (p.initValue != null && p.initText != null)
            {
