(function ($)
{
    ...
    {
        var p = options || {};
        var ashxUrl = options.ashxUrl || "../Include/UseData.asp?";
        var url = p.url || ashxUrl + $.param({ ProcessType: p.type});
        $.ajax({
            cache: false,
            async: true,
            url: url,
            ...
            success: function (result)
            {
                ...
                {
                    if (p.success)
                        p.success(result.Data, result.messages);
                }
                else
                {
                    if (p.error)
                        p.error(result.messages);
                }
            },
...
//        var MenuNo = LG.getPageMenuNo();
				var locurl=window.location.pathname;//.substring(window.location.pathname.lastIndexOf("/")+1);
        LG.ajax({
            loading: '正在加载工具条中...',
            type: 'GetMyButtons',
            method: 'GetMyButtons',
            data: { HttpContext: true, MenuUrl: locurl },
            success: function (data)
            {
...
//        var parentTabid = menuitem.attr("tabid");
        ...
        {
					$("iframe[name='" + parentTabid + "']")[0].contentWindow.liger.managers.maingrid4.loadData();
//            top.tab.reload(parentTabid);
    ...
    LG.setGridDoubleClick = function (grid, btnID, btnItemClick)
    {
		...
		LG.getMonthWeek = function (a, b, c) { 
		/* 
		a = d = 当前日期 
