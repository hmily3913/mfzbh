...
$.ligerui.controls.Grid.prototype.showFilter = function ()
{
    ...
    $(g.columns).each(function ()
    {
        ...
        var isNumber = this.type == "int" || this.type == "number" || this.type == "float"||this.type== "digits";
    ...
    function show()
    {
        ...
        else
        {
			...
			{ 
				grid.select(rowdata);
				toSelect(); 
				if(options.onseled)options.onseled(rowdata);
				win.hide();
			});
            ...
            win = $.ligerDialog.open({
                ...
                buttons: [
                 { text: '选择', onclick: function (item, dialog) {
									 toSelect();
									 if(options.onseled)options.onseled(grid.getSelected());
									 dialog.hide(); 
								 }},
                 { text: '取消', onclick: function (item, dialog) { dialog.hide(); } }
                ]
            ...
            if (options.search)
            {
                ...
                $("ul:first", formPanle).append(containerBtn1).append(containerBtn2).after('<div class="l-clear"></div>');
								//add by zbh 2012-09-15 取消回车自动提交表单的错误
								$(options.search.fields).each(function (index, domEle){
									$('#'+domEle.name).keydown(function(event){ 
//										alert(event.keyCode);
